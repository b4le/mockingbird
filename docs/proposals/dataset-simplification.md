...
| **F2** — `Conversation.audioReference` removal | **Parked (data-gated)** | Field still load-bearing in the committed fixtures: severance has one audio-only conversation (`conversation-d738b9e8-…`) and demo has one (`c5`), both with no `transcriptId` and no transcript back-linking via `conversationId`, so `resolveAudioReference` would have no fallback. Unblock path: once every audio-bearing conversation has a paired Transcript (re-sync `data/severance` from the latest atticus-finch export and confirm the audio-only set is empty), the field becomes a safe one-line drop. |
| **F3** — Transcript derivable fields | **Resolved** | `cueCount`, `hasCues`, and raw `participants` dropped from schema + interface. Re-audit confirmed `cueCount === cues.length` and `hasCues === cues.length > 0` for all 17 transcripts, and that nothing in `src/` reads any of the three (speaker resolution uses `cues`/`speakerMap`/`cue.speakerId`; invariants use resolved `participantIds`). Not actually exporter-blocked — zod `.strip()` drops the on-disk fields, same as F6. |
...
| **F9** — Derive `Stakeholder.initials` from `name` | **Resolved** | Field dropped; `getStakeholderInitials` in `src/lib/stakeholders.ts` derives at render time. The derivation reproduces all 25 real stakeholders' stored values exactly; fully bracketed names (`[Privileged]`) return the `--` redaction sentinel, preserving the privilege marker without a stored field. |
